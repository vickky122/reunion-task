...
const DataTable = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Created At',
        Cell: ({ cell }) => dayjs(cell.getValue()).format('DD-MMM-YYYY HH:mm'),
      },
      {
  ...
  const filteredData = globalFilter
    ? fuse.search(globalFilter).map((result) => result.item)
    : data;
  ...
  return (
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center">
        ...
        />
        <Button variant="outlined" onClick={() => setColumnVisibility({})}>
          Reset Columns
        </Button>
      </Box>
